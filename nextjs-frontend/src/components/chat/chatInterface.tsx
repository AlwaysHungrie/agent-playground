...
const UserMessage = ({ message }: { message: string }) => {
  return (
    <div className="flex justify-end">
      <div className="bg-primary text-primary-foreground rounded-lg px-4 py-2 max-w-[80%]">{message}</div>
    </div>
  );
};
...
const BotMessage = ({ message }: { message: string }) => {
  return (
    <div className="flex justify-start">
      <div className="bg-white border border-gray-200 rounded-lg px-4 py-2 max-w-[80%]">{message}</div>
    </div>
  );
};
...
const ChatInterface = () => {
  const [messages, setMessages] = useState([
    ...
    { id: 2, sender: 'user', text: 'I have a question about React.' },
    { id: 3, sender: 'bot', text: 'Sure! I\'d be happy to help you with React. What would you like to know?' },
  ]);
